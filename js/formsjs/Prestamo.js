/**
 * Prestamo.js
 * Datalist hidden-field mapping + "En sala" date sync logic
 */
$(document).ready(function() {
    ...
    // ==================== "EN SALA" DATE SYNC ====================
    var $tPrestamo = $('select[name="T_prestamo"]');
    var $fPrestamo = $('input[name="F_prestamo"]');
    var $fDevolucion = $('input[name="F_devolucion"]');

    function syncEnSala() {
        if (!$tPrestamo.length || !$fPrestamo.length || !$fDevolucion.length) return;
        
        if ($tPrestamo.val() === 'En sala') {
            $fDevolucion.val($fPrestamo.val());
            $fDevolucion.prop('readonly', true);
        } else {
            $fDevolucion.prop('readonly', false);
        }
    }

    $tPrestamo.on('change', syncEnSala);
    $fPrestamo.on('change', function() {
        if ($tPrestamo.val() === 'En sala') {
            $fDevolucion.val($fPrestamo.val());
        }
    });

    syncEnSala(); // Initial state
